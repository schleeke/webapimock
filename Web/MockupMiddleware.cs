    ...
    public class MockupMiddleware {
        ...
        public async Task InvokeAsync(HttpContext context) {
            ...
            if (!context.Request.Path.HasValue
              || context.Request.Path.Value.IndexOf($"/{Program.MockupRelativePath}", StringComparison.InvariantCultureIgnoreCase) != 0) {
                await _next(context);
            ...
            await context.Response.CompleteAsync();
            Program.Logger.Debug($"[{transactionId}] Successfully mocked request.");
        }
    }
}
